...
Теперь, когда `TownSerializer` в числе прочих данных получит из записи в БД список `writers`, он передаст его вложенному сериалайзеру `WriterSerializer`, обработает каждый объект в списке и вернет список словарей с интересующей нас информацией об авторах, родившихся в конкретном городе.  
